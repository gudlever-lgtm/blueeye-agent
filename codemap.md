...
| Transaction tests | [`src/transactions/`](src/transactions/) — server pushes `transaction_config` over WS; the manager schedules each http/tcp/dns/icmp test (`interval_sec` ±10% jitter), runs an executor (Node core `http`/`https`/`net`/`dns` + system `ping`), classifies the failure phase, buffers results (max 1000, oldest dropped) and flushes `transaction_result` batches on reconnect. Config persists to a local JSON file with secrets AES-256-GCM-encrypted (key derived from the token). `run-transaction` runs one NOW, out of band |
| Phase timings | [`transactions/phases.js`](src/transactions/phases.js) — each step's time split into **dns / tcp / tls / ttfb / transfer** from the events a Node socket already emits. `tcp` IS the network round-trip time; `ttfb` is the server thinking. That split is what separates a network fault from an application one, and it costs no privilege and works on every platform. A `null` phase means the moment never happened (no TLS, or a reused keep-alive socket — `reused:true`), never that it took no time |
| Header capture | [`src/capture/`](src/capture/) — the layer below the timings, for what they cannot see: retransmissions, dup-ACKs, resets, zero windows, MSS mismatch. **The filter is derived from the test, never typed** ([`filter.js`](src/capture/filter.js): every address through `net.isIP`, every port an integer; no filter ⇒ refusal, never a wider capture). snaplen 96, FIXED. `tcpdump -w -` into a pipe, decoded to named header fields ([`decode.js`](src/capture/decode.js)) and the buffer dropped — **nothing is written to disk and no payload bytes leave the decoder** (`payload` is the byte count). Under `capture:'on_fault'` the capture runs every run and is kept only when the run went wrong ([`capturePolicy.js`](src/transactions/capturePolicy.js)); everything else is discarded where it was made. Linux + tcpdump + CAP_NET_RAW, else `capabilities.unavailable.capture` says why |
| SNMP topology | [`snmpTopology.js`](src/snmpTopology.js) + [`snmpPoller.js`](src/snmpPoller.js) — the agent polls the **switches the server assigns to it** (`snmpTargets` in the agent config) for the forwarding table, LLDP neighbours and VLAN names (plus `sysName`/`sysDescr`/`sysUpTimeTicks` per device), ALONGSIDE its own traffic sampling. Breaks the old 1:1 binding where `monitorConfig.source='snmp'` made a whole agent poll one device. **Bridge port is not ifIndex** — `dot1dBasePortIfIndex` is walked and resolved before anything is reported. The poller's scheduled ticks run the RUNTIME's cycle wrapper (`start({ cycle })`), the same one `poll-snmp` uses: interface names feed the trap resolver, the diagnose timestamps move, and a 401 on the submit is fatal |
| Device events | [`src/syslog/`](src/syslog/) — the agent is the **syslog collector**: devices on the customer's own network point their logging at it (udp+tcp **1514**, off by default), it parses RFC 3164/5424 per line, classifies the fault, masks credentials and batches to `POST /agents/me/device-events`. Bounded buffer + per-sender token bucket, so a switch in an STP loop cannot make the agent the outage. ONE flush timer (`syslogFlushIntervalMs`) serves syslog and traps and starts when EITHER binds |
| SNMP traps | [`src/traps/`](src/traps/) — udp **1162**, off by default. Same start/drain/stats/stop contract as the syslog receiver, so both drain into ONE device-event batch. A trap is accepted only from an address this agent actually polls — v2c is unauthenticated and the source address is the strongest check it permits — and that is checked BEFORE any decoding. A target configured by **hostname** is resolved (`dns.lookup`, all addresses; injectable `lookupHost`) whenever the targets are applied, so its traps match; a failed lookup keeps the last known address |
...
| `path_mtu` | [`probes/pathmtu.js`](src/probes/pathmtu.js) | largest packet the path carries, per hop — binary search on packet size with the DF bit set. Separates a normal MTU reduction (ICMP frag-needed received) from a **PMTUD blackhole** (large packets vanish silently, so no sender ever learns), and both from a hop that simply doesn't answer ICMP. A hop is measured by pinging the TARGET with the TTL limited to it, so the measured MTU is monotonic along the path and `mtu_drop_at_hop` names the router that narrows it. IPv4 and IPv6 alike, per hop. Optional Linux-only MSS check via `ss -tin`. Reports `ok: true` even on a blackhole — the finding is about the path, not the agent. |
| `tls` | [`probes/tls.js`](src/probes/tls.js) | the certificate a PORT presents — not only a web server's. Opens a TLS handshake (nothing sent, nothing read) and reports the four faults APART: `expiryDays`, chain trust (`authorized` + node's own `authorizationError` code, kept verbatim), hostname match (RFC 6125 — SAN decides, one wildcard label, subject CN only when there is no SAN), and the negotiated protocol/cipher. `rejectUnauthorized: false` is deliberate: a refused handshake cannot say WHICH fault it was, and reporting that is the job. An explicit `servername` checks the certificate a particular virtual host serves (point it at an IP and name the host); an IP with no servername reports `hostnameMatches: null` rather than a verdict it never reached. `lossPct` stays 0 — a certificate fault is not packet loss, and must not land in the outage numbers. |
...
- **run-discovery** (`run[\s_-]?discovery|discovery[\s_-]?sweep|sweep` + a `discovery` object
  ...
  20000 DNP3, 44818 EtherNet/IP, 4840 OPC UA), rDNS; rate-limited, scope-capped), and
