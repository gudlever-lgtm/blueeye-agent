...
| `path_mtu` | [`probes/pathmtu.js`](src/probes/pathmtu.js) | largest packet the path carries, per hop — binary search on packet size with the DF bit set. Separates a normal MTU reduction (ICMP frag-needed received) from a **PMTUD blackhole** (large packets vanish silently, so no sender ever learns), and both from a hop that simply doesn't answer ICMP. A hop is measured by pinging the TARGET with the TTL limited to it, so the measured MTU is monotonic along the path and `mtu_drop_at_hop` names the router that narrows it. Optional Linux-only MSS check via `ss -tin`. Reports `ok: true` even on a blackhole — the finding is about the path, not the agent. |
